...
usage() {
  ...
  Beispielaufruf:        ./createHolding.sh -f ~/folio-mig/sample_input/holdings/10000001.json
...
curl $curlopts -S -X POST -H "$TOKEN" -H "X-Okapi-Tenant: $TENANT" -H "Content-type: application/json; charset=utf-8" -H "Accept: application/json" -d \@$folio_json_datei $OKAPI/holdings-storage/holdings
echo
