...
usage() {
  ...
  1. Aufruf ohne Optionen : Löscht ein Instanzbeziehung anhand ihrer ID.
     Aufruf:                ./deleteInstanceRelationship.sh ID
     benötigt: login.json im gleichen Verzeichnis.
     Beispielaufruf:        ./deleteInstanceRelationship.sh 7433c70e-8887-550b-a048-0e421ad628f2
  2. Aufruf mit Parameteroption -f : Löscht eine Instanzbeziehung anahnd einer Datei im Format FOLIO-JSON.
     ...
     Beispielaufruf:        ./deleteInstanceRelationship.sh -f ~/folio-mig/sample_input/instanceRelationships/1891_IR1.json
...
curl $curlopts -S -X DELETE -H "$TOKEN" -H "X-Okapi-Tenant: $TENANT" -H "Content-type: application/json; charset=utf-8" $OKAPI/instance-storage/instance-relationships/$id
echo
