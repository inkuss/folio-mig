#!/bin/bash
# Autor: I. Kuss, hbz
# Erzeugt Ladedateien mit FOLIO-Lokaldatensätzen

usage() {
  cat <<EOF
  Erzeugt Ladedateien mit FOLIO-Lokaldatensätzen
  Beispielaufruf:        ./createHoldingsLoadfiles.sh -d ~/folio-mig/sample_input/holdings

  Optionen:
   - d [Verzeichnis] Verzeichnis mit Lokaldaten (im Format: FOLIO-JSON)
   - h              Hilfe (dieser Text)
EOF
  exit 0
  }
...
paket_nr=0
paket_zaehler=0
# Schreibe Lokaldatensätze in eine Datei
loadfile_basename="loadfile"
loadfile=""
for holdings in $inputDir/*.json; do
...
HEAD
  ...
  cat $holdings >> $inputDir/$loadfile
done
...
echo "INFO: Anzahl insgesamt paketierter Lokaldatensätze: $zaehler"
