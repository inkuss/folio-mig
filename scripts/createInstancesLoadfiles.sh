#!/bin/bash
# Autor: I. Kuss, hbz
# Erzeugt Ladedateien mit FOLIO-Titeldatensätzen

usage() {
  cat <<EOF
  Erzeugt Ladedateien mit FOLIO-Titeldatensätzen
  Beispielaufruf:        ./createInstancesLoadfiles.sh -d ~/folio-mig/sample_input/instances

  Optionen:
   - d [Verzeichnis] Verzeichnis mit Titeldaten (im Format: FOLIO-JSON)
   - h              Hilfe (dieser Text)
EOF
  exit 0
  }
...
paket_nr=0
paket_zaehler=0
# Schreibe Titeldatensätze in eine Datei
...
HEAD
  ...
  cat $instance >> $inputDir/$loadfile
done
...
echo "INFO: Anzahl insgesamt paketierter Titeldatensätze: $zaehler"
